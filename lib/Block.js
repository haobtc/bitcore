...
Block.prototype.merkleBranchBuf = function(branch) {
  var bufs = [util.varIntBuf(branch.hashes.length)];
  bufs = bufs.concat(branch.hashes);
  var maskBuffer = new Buffer(4);
  maskBuffer.writeUInt32LE(branch.side_mask, 0);
  bufs.push(maskBuffer);
...
Block.prototype.serialize = function serialize() {
  var bufs = [this.getHeader()];
  if(this.version & (1 << 8)) {
    bufs.push(this.auxPow.coinbaseTx.serialize());
    bufs.push(this.auxPow.block_hash);
    bufs.push(this.merkleBranchBuf(this.auxPow.coinbaseBranch));
    bufs.push(this.merkleBranchBuf(this.auxPow.blockchainBranch));
    bufs.push(this.auxPow.blockHeader);
  }
  bufs.push(util.varIntBuf(this.txs.length));
  bufs = bufs.concat(this.txs.map(function(tx) {return tx.serialize();}));
  return Buffer.concat(bufs);
};
